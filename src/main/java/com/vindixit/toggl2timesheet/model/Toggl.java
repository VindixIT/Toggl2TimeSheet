...
import com.vindixit.toggl2timesheet.util.DelayFilter;
...
public class Toggl {
    ...
    public final static String TIME_ENTRIES = API_BASE + "time_entries";
     ...
     */
    public Toggl(String user, String password) {
        this.user = user;
     ...
     */
    public List<com.vindixit.toggl2timesheet.model.Client> getClients() {
        Client client = prepareClient();
        ...
        List<com.vindixit.toggl2timesheet.model.Client> clients = new ArrayList<com.vindixit.toggl2timesheet.model.Client>();
        if (data != null) {
	        for (Object obj : data) {
	            JSONObject entryObject = (JSONObject) obj;
	            clients.add(new com.vindixit.toggl2timesheet.model.Client(entryObject.toJSONString()));
	        }
        }
        return clients;
    }
     ...
     */
    public com.vindixit.toggl2timesheet.model.Client createClient(com.vindixit.toggl2timesheet.model.Client clientObject) {
        Client client = prepareClient();
        WebResource webResource = client.resource(CLIENTS);
        ...
        object = (JSONObject) JSONValue.parse(response);
        JSONObject data = (JSONObject) object.get(DATA);
        return new com.vindixit.toggl2timesheet.model.Client(data.toJSONString());
    }
     ...
     */
    public com.vindixit.toggl2timesheet.model.Client updateClient(com.vindixit.toggl2timesheet.model.Client clientObject) {
        Client client = prepareClient();
     ...
     * Get current user.
     * 
     * @return current user {@link TogglUser}
     */
    public TogglUser getCurrentUser() {
        Client client = prepareClient();
        ...
        return new TogglUser(data.toJSONString());
    }
     ...
     * @return current user {@link TogglUser}
     */
    public TogglUser getCurrentUserWithRelatedData() {
	 ...
	 * @return all users
	 */
	public List<TogglUser> getWorkspaceUsers(long workspaceId) {
		Client client = prepareClient();
		...
		List<TogglUser> users = new ArrayList<TogglUser>();
		if (data != null) {
			for (Object obj : data) {
				JSONObject entryObject = (JSONObject) obj;
				users.add(new TogglUser(entryObject.toJSONString()));
			}
		}
		return users;
	}
	 ...
	 * @return all clients
	 */
	public List<com.vindixit.toggl2timesheet.model.Client> getWorkspaceClients(long workspaceId) {
		Client client = prepareClient();
	...
	public List<TogglUser> getUsers() {
		HashSet<TogglUser> result = new HashSet<TogglUser>();
		List<Workspace> workspaces = getWorkspaces();
		for (Workspace workspace : workspaces) {
			List<TogglUser> workspaceUsers = getWorkspaceUsers(workspace.getId());
			result.addAll(workspaceUsers);
		}
		return new ArrayList<TogglUser>(result);
	}
    ...
    private JSONObject createClientRequestParameter(com.vindixit.toggl2timesheet.model.Client client) {
        JSONObject object = new JSONObject();
